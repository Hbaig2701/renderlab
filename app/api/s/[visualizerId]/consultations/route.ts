...
// POST /api/s/{visualizerId}/consultations - Create a new consultation
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    // Fetch visualizer and verify it's active with sales tool enabled
    ...
    if (visualizer.status !== 'active' || !visualizer.sales_tool_enabled) {
    ...
    // Upload to storage
    const storagePath = `${visualizerId}/${consultationId}/original.${extension}`;
...
// GET /api/s/{visualizerId}/consultations - List consultations
