...
export default function ConsultationViewPage() {
  ...
  useEffect(() => {
    fetchConsultation();
  }, [widgetId, consultationId]);

  const fetchConsultation = async () => {
    try {
      // Fetch consultation
      const res = await fetch(`/api/s/${widgetId}/consultations/${consultationId}`);
      if (!res.ok) {
      ...
      // Fetch widget for brand color
      const widgetRes = await fetch(`/api/s/${widgetId}`);
      if (widgetRes.ok) {
        const widgetData = await widgetRes.json();
        setBrandColor(widgetData.widget.brand_color);
      }
    } catch {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0A0A0A] p-4">
        <Card className="p-8 text-center max-w-md bg-[#1A1A1A] border-[#2E2E2E]">
          ...
          <Button
            onClick={() => router.push(`/s/${widgetId}`)}
            className="mt-4"
            variant="outline"
          >
