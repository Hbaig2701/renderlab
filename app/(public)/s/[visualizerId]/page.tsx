...
export default function SalesToolPage() {
  ...
  const isHairVisualizer = visualizer?.template === 'hair';
  const availableCategories = isHairVisualizer
    ? [...new Set(visualizer?.options.map(o => o.category).filter((c): c is string => !!c))]
    : [];
  const displayOptions = isHairVisualizer
    ? visualizer?.options.filter(o => o.category === selectedCategory) || []
    : visualizer?.options || [];
  ...
  const fetchVisualizerData = async () => {
    try {
      const res = await fetch(`/api/s/${visualizerId}`);
      if (!res.ok) {
        const data = await res.json();
        setError(data.error || 'Visualizer not found');
        setLoading(false);
        return;
      }
      const data = await res.json();
      setVisualizer(data.widget);
      setTier(data.tier);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      // Create consultation
      try {
        const res = await fetch(`/api/s/${visualizerId}/consultations`, {
          method: 'POST',
  ...
  const handleTransform = async () => {
    ...
    try {
      const res = await fetch(`/api/s/${visualizerId}/consultations/${consultationId}/transform`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`/api/s/${visualizerId}/consultations/${consultationId}/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: sendEmail,
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="w-full max-w-4xl">
            <BeforeAfterSlider
              ...
              brandColor={visualizer.brand_color}
            />
          </div>
        </div>
      ...
      <div className="max-w-lg mx-auto space-y-6">
        ...
        {visualizer.logo_url && (
          <div className="flex justify-center">
            <img src={visualizer.logo_url} alt="Logo" className="h-12 object-contain" />
          </div>
        )}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {consultations.map((c) => (
                    <button
                      key={c.id}
                      onClick={() => router.push(`/s/${visualizerId}/c/${c.id}`)}
        ...
        {screen === 'new' && (
          <>
            ...
            <Card className="p-8 border-2 border-dashed bg-[#1A1A1A]" style={{ borderColor: visualizer.brand_color }}>
              <div className="text-center space-y-4">
                <div
                  className="w-16 h-16 rounded-full flex items-center justify-center mx-auto"
                  style={{ backgroundColor: visualizer.brand_color + '20' }}
                >
                  <Camera className="h-8 w-8" style={{ color: visualizer.brand_color }} />
                </div>
                <div>
        ...
        {screen === 'style' && originalImage && (
          <>
            ...
            {isHairVisualizer && availableCategories.length > 0 && (
                ...
                {availableCategories.map((category) => {
                  ...
                  return (
                    <button
                      ...
                      style={selectedCategory === category ? { backgroundColor: visualizer.brand_color } : {}}
                    >
            ...
            {/* Options */}
            {(isHairVisualizer ? selectedCategory : true) && displayOptions.length > 0 && (
              <div className="grid grid-cols-2 gap-3">
                {displayOptions.map((option) => (
                  <button
                    ...
                    style={selectedOption === option.key ? { backgroundColor: visualizer.brand_color } : {}}
                  >
        ...
        {screen === 'loading' && (
          <div className="flex flex-col items-center justify-center py-20 space-y-6">
            <Loader2 className="h-12 w-12 animate-spin" style={{ color: visualizer.brand_color }} />
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div
                  ...
                  style={{ borderColor: visualizer.brand_color }}
                >
