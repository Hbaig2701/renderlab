...
export function ROIReportButton({
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      doc.text('Widget Performance Report', pageWidth - 20, 25, { align: 'right' });
      ...
      doc.setFontSize(10);
      doc.setTextColor(...mutedColor);
      doc.text('Total Transforms', 20 + boxWidth / 2, statsY + 12, { align: 'center' });
      doc.setFontSize(24);
      doc.setTextColor(...primaryColor);
      doc.setFont('helvetica', 'bold');
      doc.text(totalTransforms.toString(), 20 + boxWidth / 2, statsY + 30, { align: 'center' });
      ...
      doc.setTextColor(...primaryColor);
      doc.setFont('helvetica', 'bold');
      doc.text(weekTransforms.toString(), 30 + boxWidth + boxWidth / 2, statsY + 30, { align: 'center' });
      ...
      const estimatedLeads = Math.round(totalTransforms * avgConversionLift);

      const roiText = [
        `Based on industry benchmarks, visualization widgets typically improve`,
        `conversion rates by 15-30%. With ${totalTransforms} total transformations:`,
        '',
      ...
      doc.setTextColor(...textColor);
      doc.text(
        'This widget helps potential customers visualize results before committing,',
        65,
        245
      );
      ...
      doc.setFontSize(9);
      doc.setTextColor(...mutedColor);
      doc.text(`Widget ID: ${widgetId}`, 20, 288);
